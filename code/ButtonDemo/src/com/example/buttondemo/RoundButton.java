...
public class RoundButton extends Button {
	...
	private void init(Context context, AttributeSet attrs) {
		TypedArray typedArray = context.getTheme().obtainStyledAttributes(
				attrs, R.styleable.RoundButton, 0, 0);

		isCircle = typedArray.getBoolean(R.styleable.RoundButton_is_circle, false);
		corner_radius = typedArray.getDimension(
				R.styleable.RoundButton_corner_radius, 0);
		
		
		solid_color = typedArray.getColor(R.styleable.RoundButton_solid_color,
				Color.TRANSPARENT);
		stroke_color = typedArray.getColor(R.styleable.RoundButton_stroke_color,
				Color.BLACK);
		stroke_width = typedArray.getDimension(R.styleable.RoundButton_stroke_width,0 * density);
		stroke_width=dpToPx(stroke_width);
		
		
		pressed_solid_color = typedArray.getColor(R.styleable.RoundButton_pressed_solid_color, solid_color);
		pressed_stroke_color = typedArray.getColor(R.styleable.RoundButton_pressed_stroke_color, stroke_color);
		pressed_stroke_width = typedArray.getDimension(R.styleable.RoundButton_pressed_stroke_width,stroke_width);
		
		selected_solid_color = typedArray.getColor(R.styleable.RoundButton_selected_solid_color, solid_color);
		selected_stroke_color = typedArray.getColor(R.styleable.RoundButton_selected_stroke_color, stroke_color);
		selected_stroke_width = typedArray.getDimension(R.styleable.RoundButton_selected_stroke_width,stroke_width);
